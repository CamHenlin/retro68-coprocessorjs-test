...
void setupPBControlForSerialPort(short serialPortShort) {
    ...
    err = PBControl ((ParmBlkPtr) & cb, 0); // PBControl definition: http://mirror.informatimago.com/next/developer.apple.com/documentation/mac/Networking/Networking-296.html
...
#define PRINTER_PORT_IN  "\p.BIn"
    ...
    OSErr err = MacOpenDriver(serialPortInputName, &serialPortInput);
...
char* readSerialPort() {
    ...
    incomingSerialPortReference.ioReqCount = 0; // TODO: we need an expected receive length. coprocessor.js needs to supply set chunk sizes?
...
OSErr writeSerialPort(const char* stringToWrite) {
    ...
    OSErr err = PBWrite((ParmBlkPtr)& outgoingSerialPortReference, 0); // second param is async, TODO: investigate
...
int main(int argc, char** argv) {
    ...
    setupCoprocessor("printer"); // could also be "modem"
